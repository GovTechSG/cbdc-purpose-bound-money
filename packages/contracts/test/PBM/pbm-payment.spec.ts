          ...
          describe(`When autoWithdrawal is ${autoWithdrawal}`, () => {
            describe("When caller is a payer", () => {
              describe("When payee is not whitelisted", () => {
                it("should revert when paid to non-whitelisted payee", async () => {
                  const nonWhitelistedPayee = fixtures.signers.others[5];

                  const tx = pbmContract
                    .connect(payer)
                    .pay(nonWhitelistedPayee.address, payingAmount, lockingPeriod, autoWithdrawal);
                ...
                describe("When payer does not have underlying asset at all", () => {
                  ...
                  it("should revert when payer attempts to pay", async () => {
                    const tx = pbmContract
                      ...
                      .pay(payee.address, payingAmount, lockingPeriod, autoWithdrawal);
                ...
                describe("When payer holds underlying asset", () => {
                  describe("When payer has sufficient underlying asset", () => {
                    it("should revert when payer has not approved", async () => {
                      ...
                      const allowance = await dsgdContract.allowance(
                        payer.address,
                        pbmContract.address,
                      );
                  ...
                  describe("When payer has insufficient underlying asset", () => {
                    it("should revert when payer does not hold enough asset", async () => {
                      ...
                      const tx = pbmContract
                        .connect(payer)
                        .pay(payee.address, exceedingAmount, lockingPeriod, autoWithdrawal);
              ...
              describe("When payment is made without locking period", () => {
                beforeEach(async () => {
                  ...
                  payTx = await pbmContract
                    .connect(payer)
                    .pay(payee.address, payingAmount, 0, autoWithdrawal);
                });
              ...
              describe("When payment is made with locking period", () => {
                ...
                describe("Transfer behaviour", () => {
                  ...
                  it("should transfer the underlying asset from payer to PBM contract", async () => {
                    const pbmContractDsgdBalance = await dsgdContract.balanceOf(
                      pbmContract.address,
                    );
                ...
                describe("Deposit record in vault", () => {
                  ...
                  it("should indicate payment as active deposit under payee address", async () => {
                    const payeeActiveDeposits = await pbmVaultContract.getActiveDepositIds(
                      payee.address,
                    );
              ...
              describe("When multiple payments are made to multiple payees", () => {
                ...
                beforeEach(async () => {
                  ...
                  await Promise.all([
                    pbmContract
                      .connect(fixtures.signers.admin)
                      .grantRole(payeeRole, payee1.address),
                    pbmContract
                      .connect(fixtures.signers.admin)
                      .grantRole(payeeRole, payee2.address),
                  ]);

                  // Assert all initial payee deposits states
                  const initialPayee1Deposits = await pbmVaultContract.getAllDepositIds(
                    payee1.address,
                  );
                  const initialPayee2Deposits = await pbmVaultContract.getAllDepositIds(
                    payee2.address,
                  );
                  ...
                  await pbmContract
                    .connect(payer)
                    .pay(payee1.address, parseAmount(100), lockingPeriod, autoWithdrawal);
                  // Pay to payee2
                  await pbmContract
                    .connect(payer)
                    .pay(payee2.address, parseAmount(200), lockingPeriod, autoWithdrawal);
                  // Pay to payee1 again
                  await pbmContract
                    .connect(payer)
                    .pay(payee1.address, parseAmount(300), lockingPeriod, autoWithdrawal);
                  // Pay to payee2 again with different a different locking period
                  await pbmContract
                    .connect(payer)
                    .pay(payee2.address, parseAmount(400), 3600 * 24 * 30, autoWithdrawal);
                  // Pay to payee1 again
                  await pbmContract
                    .connect(payer)
                    .pay(payee1.address, parseAmount(500), lockingPeriod, autoWithdrawal);
                });
                ...
                it("should return the correct number of payee's total deposits", async () => {
                  const totalPayee1Deposits = await pbmVaultContract.totalDepositsOf(
                    payee1.address,
                  );
                  const totalPayee2Deposits = await pbmVaultContract.totalDepositsOf(
                    payee2.address,
                  );
                ...
                it("should return the correct active deposit IDs of payees", async () => {
                  const payee1ActiveDeposits = await pbmVaultContract.getActiveDepositIds(
                    payee1.address,
                  );
                  const payee2ActiveDeposits = await pbmVaultContract.getActiveDepositIds(
                    payee2.address,
                  );
                ...
                describe("Deposit Pagination", () => {
                  it("should return the deposit IDs of all payees correctly", async () => {
                    const payee1Deposits = await pbmVaultContract.getDepositIds(
                      payee1.address,
                      0,
                      3,
                    );
                    const payee2Deposits = await pbmVaultContract.getDepositIds(
                      payee2.address,
                      0,
                      2,
                    );
            ...
            describe("When payment parameters do not pass validation check", () => {
              ...
              it("should revert if payee address is zero", async () => {
                ...
                const tx = pbmContract
                  .connect(payer)
                  .pay(ethers.constants.AddressZero, payingAmount, lockingPeriod, autoWithdrawal);

...
interface PaymentsListingTableProps extends React.ComponentProps<typeof PaymentsTable> {}

const canRecall = (statuses: PaymentStatus[]) => statuses.includes(PaymentStatus.Withheld)

export const PaymentsListingTable = forwardRef<
    PaymentsTableForwardedRef,
    PaymentsListingTableProps
>(function PaymentsListingTable(props, ref) {
    const { decimals, onChargeback } = props

    const { isConnectedTreasurer } = useAccountAccessContext()
                ...
                <>
                    ...
                    <ColumnGroup title="Addresses">
                        ...
                        <Column
                            ...
                            filteredValue={filteredInfo.toAddress || null}
                            render={addressLabelRenderer}
                        />
                    </ColumnGroup>
                    <Column
                        ...
                        render={(_: any, record: PaymentDataSource) => (
                            <Space size="middle">
                                <Button
                                    type="link"
                                    size="small"
                                    disabled={!canRecall(record.statuses) || !isConnectedTreasurer}
